...
public class D8ClassConverter implements ClassVisitor {
  ...
  public void visitProgramClass(ProgramClass programClass) {
    try {
      ...
      programClass.accept(new ProgramClassWriter(new DataOutputStream(byteArrayOutputStream)));
    ...
    public D8DexFile(ClassPath libraryJars, int minSdkVersion, boolean debuggable) {
      ...
      if (libraryJars != null) {
        ...
        for (int index = 0; index < libraryJars.size(); index++) {
          ...
          if (Files.exists(classPathEntry.getFile().toPath()) && (classPathEntry.isDex())
