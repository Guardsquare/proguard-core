...
public class DexClassReader implements DataEntryReader {
   ...
   *
   * <p>Does not generate {@link PrimitiveArrayConstant}s by default, which is a custom ProGuardCORE
   * extension.
   *
   ...
   * @param classVisitor the class visitor to which decoded classes will be passed.
   */
  public DexClassReader(
      boolean readCode, boolean usePrimitiveArrayConstants, ClassVisitor classVisitor) {
  ...
  /**
   * This main method illustrates and tests the class. It reads an input dex file (or jar file with
   * a dex file) and prints out its classes.
   */
  public static void main(String[] args) throws Exception {
    String fileName = args.length > 0 ? args[0] : "classes.dex";

    DataEntryReader reader = new DexClassReader(true, new ClassPrinter());

    if (!fileName.endsWith(".dex")) {
      reader = new JarReader(new NameFilteredDataEntryReader("**.dex", reader));
    }
