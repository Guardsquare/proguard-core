...
public abstract class DexDataEntryWriter implements DataEntryWriter {
   ...
   *
   * @param classPool the class pool from which classes are collected.
   * @param classNameFilter an optional filter for classes to be written.
   * @param dexFileName the dex file name.
   * @param forceDex specifies whether the dex files should always be written, even if they don't
   *     contain any code.
   * @param dexDataEntryWriter the writer to which the converted dex file is written.
   * @param otherDataEntryWriter the writer to which other data entries are written.
   */
  public DexDataEntryWriter(
      ClassPool classPool,
  ...
  public OutputStream createOutputStream(DataEntry dataEntry) throws IOException {
    ...
    if (name.endsWith(ClassConstants.CLASS_FILE_EXTENSION)) {
      ...
      String className =
          name.substring(0, name.length() - ClassConstants.CLASS_FILE_EXTENSION.length());
      if (classNameFilter == null || classNameFilter.matches(className)) {
  ...
  private void finishIfNecessary(DataEntry dataEntry) throws IOException {
    ...
    if (currentDexEntry != null
        && !dexDataEntryWriter.sameOutputStream(
            currentDexEntry, new DexDataEntry(dataEntry, dexFileName))) {
      finish();
    }
  }
  ...
  private void finish() throws IOException {
    ...
    if (currentDexEntry != null) {
      OutputStream outputStream = dexDataEntryWriter.createOutputStream(currentDexEntry);
