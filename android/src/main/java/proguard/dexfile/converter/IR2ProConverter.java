...
public class IR2ProConverter {
  ...
  /**
   * If set to >0, this config option has the effect that methods that contain overly complex
   * statements with more than this amount of nested expressions are not converted to Java bytecode,
   * as the conversion process would most likely produce a {@link StackOverflowError}. This
   * workaround results in a {@link proguard.classfile.Method} with an empty {@link CodeAttribute}
   * if it contains such a deeply nested statement.
   */
  private static final int MAX_STATEMENT_DEPTH =
      Integer.parseInt(System.getProperty("proguard.dexconversion.maxstatementdepth", "0"));
  ...
  /**
   * Specifies whether {@link PrimitiveArrayConstant}s can be generated, when applicable.
   *
   * <p>If they are generated then they should be converted back to standard Java arrays before
   * converting to Java class files using {@link PrimitiveArrayConstantReplacer}.
   */
  ...
  public void convert() {
    ...
    try {
      ...
      code.reset();
      code.beginCodeFragment(
          10); // start dummy fragment such that the subsequent endCodeFragment call succeeds
      return;
    }
  ...
  /**
   * an empty try-catch block will cause other crash, we check this by finding non-label stmts
   * between {@link Trap#start} and {@link Trap#end}. if find we add the try-catch or we drop the
   * try-catch.
   *
   * @param ir
   * @param code
    ...
    for (Trap trap : ir.traps) {
      ...
      if (needAdd) {
        for (int i = 0; i < trap.handlers.length; i++) {
          ...
          code.catch_(
              (CompactCodeAttributeComposer.Label) trap.start.tag,
  ...
  private CompactCodeAttributeComposer reBuildInstructions(
      IrMethod ir, CompactCodeAttributeComposer code) {
  ...
  private CompactCodeAttributeComposer reBuildInstructions(
      IrMethod ir,
      Stmt st,
      Map<String, Integer> lockMap,
      int[] mutableMaxLocalIndex,
      CompactCodeAttributeComposer code) {
    switch (st.st) {
      case LABEL:
        {
            ...
            case LOCAL:
              ...
              if (v2.vt == VT.LOCAL && (i == ((Local) v2)._ls_index)) { // check for a=a
                ...
                if (v2.vt == VT.ADD) {
                  ...
                  } else if (isLocalWithIndex(v2.getOp2(), i)
                      && v2.getOp1().vt == VT.CONSTANT) { // a=1+a;
      ...
      case FILL_ARRAY_DATA:
        {
          ...
          if (e2.getOp2().vt == VT.CONSTANT) {
            ...
            String elementType =
                arrayValueType.charAt(0) == '[' ? arrayValueType.substring(1) : "I";
          ...
          if (optimizeSynchronized) {
            switch (v.vt) {
              ...
              case CONSTANT:
                {
                  String key =
                      v.vt == VT.LOCAL ? "L" + ((Local) v)._ls_index : "C" + ((Constant) v).value;
                  Integer integer = lockMap.get(key);
                  return integer != null ? xload(integer, v, code) : accept(v, code);
                }
                // TODO other
              default:
      ...
      case LOOKUP_SWITCH:
        {
          ...
          return code.lookupswitch(
              (CompactCodeAttributeComposer.Label) lss.defaultTarget.tag,
              lss.lookupValues,
              targets);
        }
      case TABLE_SWITCH:
        {
          ...
          return code.tableswitch(
              (CompactCodeAttributeComposer.Label) tss.defaultTarget.tag,
              tss.lowIndex,
   ...
   * @param code
   */
  private CompactCodeAttributeComposer insertI2x(
      String tos, String expect, CompactCodeAttributeComposer code) {
  ...
  private CompactCodeAttributeComposer reBuildJumpInstructions(
      IfStmt st, CompactCodeAttributeComposer code) {
    ...
    switch (type.charAt(0)) {
      ...
      case 'L':
        ...
        if (isZeroOrNull(v1) || isZeroOrNullV2) { // IF[non]null
          ...
          return v.vt == VT.EQ ? code.ifnull(target) : code.ifnonnull(target);
        } else {
          accept(v1, code);
          accept(v2, code);
          return v.vt == VT.EQ ? code.ifacmpeq(target) : code.ifacmpne(target);
        }
      default:
        // IFx
        // IF_ICMPx
  ...
  private CompactCodeAttributeComposer pushConstant(Object cst, CompactCodeAttributeComposer code) {
    ...
    throw new UnsupportedOperationException(
        "Unsupported constant " + cst.getClass().getName() + " [" + cst + "]");
  }

  private CompactCodeAttributeComposer reBuildEnExpression(
      EnExpr value, CompactCodeAttributeComposer code) {
    ...
    switch (value.vt) {
      ...
      case INVOKE_NEW:
        code.new_(toInternal(((InvokeNewExpr) value).getClassName())).dup();
        // pass through
      ...
      case INVOKE_CUSTOM:
        {
          ...
          int bootStrapMethodIndex =
              Dex2Pro.convertBootstrapMethod(
                  code.getTargetClass(), code.getConstantPoolEditor(), ice.handle, ice.bsmArgs);
      ...
      case INVOKE_POLYMORPHIC:
        {
          ...
          return code.invokevirtual(
              toInternal(m.getOwner()), m.getName(), ipe.getProto().getDesc());
        }
      default:
        throw new RuntimeException(value.vt.toString());
    }
  ...
  /** Returns whether the given array is a primitive array with only constants. */
  private boolean isConstantPrimitiveArray(FilledArrayExpr fae) {
    ...
    if (fae.valueType.length() != 2) return false;
  ...
  /** Creates a primitive array of the given type (for example "[I") and length. */
  ...
  private CompactCodeAttributeComposer box(
      String provideType, String expectedType, CompactCodeAttributeComposer code) {
  ...
  private CompactCodeAttributeComposer reBuildE1Expression(
      E1Expr e1, CompactCodeAttributeComposer code) {
  ...
  private CompactCodeAttributeComposer reBuildE2Expression(
      E2Expr e2, CompactCodeAttributeComposer code) {
        ...
        case 'I':
          {
            ...
            if (s < 0) {
              code.ldc(-s);
              return e2.vt == VT.ADD ? code.isub() : code.iadd();
            }
            break;
          }
        case 'F':
          {
            ...
            if (s < 0) {
              code.ldc(-s);
              return e2.vt == VT.ADD ? code.fsub() : code.fadd();
            }
            break;
          }
        case 'J':
          {
            ...
            if (s < 0) {
              code.ldc2_w(-s);
              return e2.vt == VT.ADD ? code.lsub() : code.ladd();
            }
            break;
          }
        case 'D':
          {
            ...
            if (s < 0) {
              code.ldc2_w(-s);
              return e2.vt == VT.ADD ? code.dsub() : code.dadd();
            }
            break;
          }
      }
  ...
  private CompactCodeAttributeComposer cast2(
      String t1, String t2, CompactCodeAttributeComposer code) {
    if (t1.equals(t2)) {
      return code;
    }
    switch (t1.charAt(0)) {
  ...
  private CompactCodeAttributeComposer xstore(
      int var, String v, CompactCodeAttributeComposer code) {
    switch (v.charAt(0)) {
      case 'L':
      case '[':
