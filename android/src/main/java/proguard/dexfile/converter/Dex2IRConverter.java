...
public class Dex2IRConverter {
  ...
  private void supplementLineNumber(DexCodeNode dexCodeNode) {
    if (dexCodeNode == null
        || dexCodeNode.debugNode == null
        || dexCodeNode.debugNode.debugNodes == null) {
      return;
    }
  ...
  private void initExceptionHandlers(DexCodeNode dexCodeNode, BitSet[] exBranch, BitSet handlers) {
    if (dexCodeNode.tryStmts != null) {
      for (TryCatchNode tcb : dexCodeNode.tryStmts) {
        ...
        if (hasEx) {
          target.traps.add(
              new Trap(getLabel(tcb.start), getLabel(tcb.end), getLabels(tcb.handler), tcb.type));
        }
      }
    }
  }
  ...
  private void dfs(
      BitSet[] exBranch, BitSet handlers, BitSet access, DvmInterpreter<DvmValue> interpreter) {
    currentEmit = preEmit;
          ...
          switch (p.op) {
            ...
            case BAD_OP:
              emit(
                  nThrow(
                      nInvokeNew(
                          new Value[] {nString("bad dex opcode")},
  ...
  private DvmInterpreter<DvmValue> buildInterpreter() {
    return new DvmInterpreter<DvmValue>() {
      ...
      public DvmValue unaryOperation(DexStmtNode insn, DvmValue value) {
        ...
        switch (insn.op) {
          ...
          case IF_EQZ:
            emit(
                nIf(
                    nEq(local, nInt(0), TypeClass.ZIL.name),
                    getLabel(((JumpStmtNode) insn).label)));
            return null;
          ...
          case IF_NEZ:
            emit(
                nIf(
                    nNe(local, nInt(0), TypeClass.ZIL.name),
                    getLabel(((JumpStmtNode) insn).label)));
            return null;
          ...
          case SPUT_WIDE:
            {
              Field field = ((FieldStmtNode) insn).field;
              emit(
                  nAssign(nStaticField(field.getOwner(), field.getName(), field.getType()), local));
              return null;
            }
          case IGET:
          case IGET_BOOLEAN:
          ...
          case AND_INT_LIT8:
            return b(
                nAnd(
                    local,
                    nInt(((Stmt2R1NNode) insn).content),
                    ((Stmt2R1NNode) insn).content < 0 || ((Stmt2R1NNode) insn).content > 1
                        ? "I"
                        : TypeClass.ZI.name));
      ...
      public DvmValue binaryOperation(DexStmtNode insn, DvmValue value1, DvmValue value2) {
        ...
        switch (insn.op) {
          ...
          case IF_EQ:
            emit(
                nIf(
                    nEq(local1, local2, TypeClass.ZIL.name),
                    getLabel(((JumpStmtNode) insn).label)));
            return null;
          ...
          case IF_NE:
            emit(
                nIf(
                    nNe(local1, local2, TypeClass.ZIL.name),
                    getLabel(((JumpStmtNode) insn).label)));
            return null;
          ...
          case IPUT_WIDE:
            ...
            emit(
                nAssign(
                    nField(local1, field.getOwner(), field.getName(), field.getType()), local2));
            return null;
      ...
      @Override
      public DvmValue ternaryOperation(
          DexStmtNode insn, DvmValue value1, DvmValue value2, DvmValue value3) {
