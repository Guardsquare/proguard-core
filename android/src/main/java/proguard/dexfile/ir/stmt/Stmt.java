...
public abstract class Stmt {
  ...
  public static enum ST {
    ...
    ASSIGN(CAN_CONTINUE | MAY_THROW),
    IDENTITY(CAN_CONTINUE),
    LABEL(CAN_CONTINUE),
    LOCK(CAN_CONTINUE | CAN_THROW),
    NOP(CAN_CONTINUE),
    UNLOCK(CAN_CONTINUE | CAN_THROW),
    VOID_INVOKE(CAN_CONTINUE | CAN_THROW),
    FILL_ARRAY_DATA(CAN_CONTINUE | CAN_THROW), //
    RETURN(MAY_THROW),
    RETURN_VOID(0),
    THROW(CAN_THROW), //
    GOTO(CAN_BRNANCH),
    IF(CAN_CONTINUE | CAN_BRNANCH | MAY_THROW), //
    LOOKUP_SWITCH(CAN_SWITCH | MAY_THROW),
    TABLE_SWITCH(CAN_SWITCH | MAY_THROW),
    ;
    private int config;
  ...
  public final ET et;
  /**
   * Used in ordering statements in a {@link TreeSet}, id of the {@link Stmt} in its {@link
   * StmtList}
   */
  public int id;
  ...
  /**
   * @return Next statement in {@link StmtList}, null if it is the last statement in {@link
   *     StmtList}
   */
  ...
  /**
   * @return Previous statement in {@link StmtList}, null if it is the first statement in {@link
   *     StmtList}
   */
