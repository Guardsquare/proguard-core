...
public class AggTransformer extends StatedTransformer {
  ...
  /**
   * dfs find find local and the first locationInsensitive Value // TODO if can not merge, try
   * adjust the stmt to fit the local
   */
  ...
  /**
   * dfs searching, if local is appear before first location-insensitive value, throws SUCCESS, or
   * throws FAIL
   */
  private static void localCanExecFirst(Local local, Value op) throws MergeResult {
    ...
    if (op.vt == Value.VT.INVOKE_STATIC) {
      ...
      if (ie.getName().equals("valueOf")
          && ie.getOwner().startsWith("Ljava/lang/")
          && ie.getArgs().length == 1
          && ie.getArgs()[0].length() == 1) {
        shouldExclude = true;
      }
    }
  ...
  /**
   * if a local is only used in one place, and the value is isLocationInsensitive, remove the local
   ...
   * </pre>
   */
  private boolean simpleMergeLocals(
      IrMethod method, boolean changed, Set<Stmt> locationSensitiveStmts) {
  ...
  static boolean isLocationInsensitive(Value op) {
    switch (op.et) {
      ...
      case E2:
        return isLocationInsensitive(op.vt)
            && isLocationInsensitive(op.getOp1())
