...
/**
 * dex does have the instruction to create a multi-array. the implement is to using the
 * Array.newInstance(). transform
 *
 * <pre>
                ...
                if (elementType != null) {
                  ...
                  if (invokeExpr.getArgs()[1].equals("I")) {
                    if (te.type.equals("[" + elementType)) {
                      ...
                      changed[0] = true;
                      if (d > 0) {
                        return Exprs.nNewMutiArray(
                            elementType.substring(d), d + 1, new Value[] {dt});
                      } else {
                    ...
                    if (dt.vt == Value.VT.FILLED_ARRAY) {
                      ...
                      if (te.type.length() > d && te.type.substring(d).equals(elementType)) {
                        ...
                        changed[0] = true;
                        return Exprs.nNewMutiArray(
                            elementType.substring(d1), d1 + d, filledArrayExpr.getOps());
                      }
                    }
                  }
                }
