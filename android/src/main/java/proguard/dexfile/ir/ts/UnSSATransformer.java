...
public class UnSSATransformer implements Transformer {
  ...
  /**
   * there is somewhere both a and its possible x is both live, insert a=x, will change the meaning
   * for example
   *
   * <pre>
   *                      L0:
   ...
   * </pre>
   *
   * <p>after insert a=b before the if stmt, the programe change to
   *
   * <pre>
   *                      L0:
   *                      // a = phi(b, ... )
   ...
   * </pre>
   *
   * <p>the solution is by introduce a new local x
   *
   * <pre>
   *                      L0:
   *                      x = phi(b, ... )
    ...
    for (LabelStmt labelStmt : phiLabels) {
      ...
      for (AssignStmt phi : phis) {
        ...
        if (introduceNewLocal) {
          ...
          if (next != null
              && next.st == Stmt.ST.IDENTITY
              && next.getOp2().vt == Value.VT.EXCEPTION_REF) {
  ...
  private void insertAssignPath(
      StmtList stmts, Stmt from, LabelStmt labelStmt, List<AssignStmt> buff) {
  ...
  protected static class LiveA extends BaseAnalyze<LiveV> {
    static Comparator<LiveV> sortByHopsASC =
        new Comparator<LiveV>() {
