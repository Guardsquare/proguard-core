...
public class DvmFrame<V> {
  ...
  public void execute(DexStmtNode insn, DvmInterpreter<V> interpreter) {
    ...
    switch (insn.op) {
      ...
      case USHR_LONG:
        ...
        setReg(
            stmt3RNode.a,
            interpreter.binaryOperation(insn, getReg(stmt3RNode.b), getReg(stmt3RNode.c)));
        setTmp(null);
        break;
      case IF_EQ:
      case IF_GE:
      ...
      case APUT_WIDE:
        ...
        interpreter.ternaryOperation(
            insn, getReg(stmt3RNode1.b), getReg(stmt3RNode1.c), getReg(stmt3RNode1.a));
        setTmp(null);
        break;
      ...
      case INVOKE_POLYMORPHIC_RANGE:
        {
          ...
          if (methodStmtNode.args.length
              != descriptorLength(proto.getParameterTypes()) + (isStatic ? 0 : 1)) {
            System.err.println(
                "WARNING: Illegal bytecode: Mismatch between argument count and method signature.");
            System.err.println(
                "         This may be the result of defining a function with more than 255 parameters.");
            System.err.println("         For invocation of " + getMethodName(methodStmtNode));
            System.err.println(
                "         This is a bug in your input, and will cause a crash at runtime.");
          }

          int startReg =
              isRange
                  ? methodStmtNode
                      .args[0] // starting register is always included in args for invoke-*/range
                  : 0; // unused dummy value, args.length may be 0 (e.g. plain invoke-static with no
          // arguments)
      ...
      case USHR_LONG_2ADDR:
        ...
        setReg(
            stmt2RNode2.a,
            interpreter.binaryOperation(insn, getReg(stmt2RNode2.a), getReg(stmt2RNode2.b)));
        setTmp(null);
        break;
      case ADD_INT_LIT16:
      case ADD_INT_LIT8:
  ...
  public static int descriptorLength(String[] types) {
    return Arrays.stream(types)
        .mapToInt(s -> s.charAt(0) == 'J' || s.charAt(0) == 'D' ? 2 : 1)
        .sum();
  }
}
