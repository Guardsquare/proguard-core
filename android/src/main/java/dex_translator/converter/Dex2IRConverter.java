package dex_translator.converter;

import dex_ir.IrMethod;
import dex_ir.Trap;
import dex_ir.TypeClass;
import dex_ir.expr.Exprs;
import dex_ir.expr.Local;
import dex_ir.expr.Value;
import dex_ir.stmt.AssignStmt;
import dex_ir.stmt.LabelStmt;
import dex_ir.stmt.Stmt;
import dex_ir.stmt.StmtList;
import dex_reader_api.DexLabel;
import dex_reader_api.DexType;
import dex_reader_api.Field;
import dex_reader_api.Method;
import dex_reader_api.node.DexCodeNode;
import dex_reader_api.node.DexDebugNode;
import dex_reader_api.node.TryCatchNode;
import dex_reader_api.node.analysis.DvmFrame;
import dex_reader_api.node.analysis.DvmInterpreter;
import dex_reader_api.node.insn.*;
import dex_reader_api.reader.Op;
import dex_reader_api.visitors.DexDebugVisitor;

import java.util.*;

import static dex_ir.expr.Exprs.*;
import static dex_ir.stmt.Stmts.*;
...
public class Dex2IRConverter {
    ...
    public IrMethod convert(boolean isStatic, Method method, DexCodeNode dexCodeNode) {
        ...
        Set<dex_ir.expr.Value> phiValues = new HashSet<>();
    ...
    private void addPhi(DvmValue v, Set<dex_ir.expr.Value> phiValues, List<AssignStmt> phis) {
        if (v != null) {
            if (v.local != null) {
                if (v.parent != null) {
                ...
                if (phiValues.size() > 0) {
                    phis.add(nAssign(v.local, nPhi(phiValues.toArray(new dex_ir.expr.Value[phiValues.size()]))));
                    phiValues.clear();
                }
            }
        }
    ...
    private DvmInterpreter<DvmValue> buildInterpreter() {
        return new DvmInterpreter<DvmValue>() {
            DvmValue b(dex_ir.expr.Value value) {
