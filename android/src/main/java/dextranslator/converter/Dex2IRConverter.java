package dextranslator.converter;

import dexir.IrMethod;
import dexir.Trap;
import dexir.TypeClass;
import dexir.expr.Exprs;
import dexir.expr.Local;
import dexir.expr.Value;
import dexir.stmt.AssignStmt;
import dexir.stmt.LabelStmt;
import dexir.stmt.Stmt;
import dexir.stmt.StmtList;
import dexreaderapi.DexLabel;
import dexreaderapi.DexType;
import dexreaderapi.Field;
import dexreaderapi.Method;
import dexreaderapi.node.DexCodeNode;
import dexreaderapi.node.DexDebugNode;
import dexreaderapi.node.TryCatchNode;
import dexreaderapi.node.analysis.DvmFrame;
import dexreaderapi.node.analysis.DvmInterpreter;
import dexreaderapi.node.insn.*;
import dexreaderapi.reader.Op;
import dexreaderapi.visitors.DexDebugVisitor;

import java.util.*;

import static dexir.expr.Exprs.*;
import static dexir.stmt.Stmts.*;
...
public class Dex2IRConverter {
    ...
    public IrMethod convert(boolean isStatic, Method method, DexCodeNode dexCodeNode) {
        ...
        Set<dexir.expr.Value> phiValues = new HashSet<>();
    ...
    private void addPhi(DvmValue v, Set<dexir.expr.Value> phiValues, List<AssignStmt> phis) {
        if (v != null) {
            if (v.local != null) {
                if (v.parent != null) {
                ...
                if (phiValues.size() > 0) {
                    phis.add(nAssign(v.local, nPhi(phiValues.toArray(new dexir.expr.Value[phiValues.size()]))));
                    phiValues.clear();
                }
            }
        }
    ...
    private DvmInterpreter<DvmValue> buildInterpreter() {
        return new DvmInterpreter<DvmValue>() {
            DvmValue b(dexir.expr.Value value) {
