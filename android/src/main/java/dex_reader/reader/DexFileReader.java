...
import dex_reader.util.Mutf8;
import dex_reader_api.*;
import dex_reader_api.node.DexAnnotationNode;
import dex_reader_api.node.analysis.DvmFrame;
import dex_reader_api.reader.InstructionFormat;
import dex_reader_api.reader.InstructionIndexType;
import dex_reader_api.reader.Op;
import dex_reader_api.visitors.*;
...
public class DexFileReader implements BaseDexFileReader {
    ...
    private static final boolean SKIP_BROKEN_DEBUG_INFO = System.getProperty("dex2jar.skip_broken_debug_information") != null;
    ...
    private void read_debug_info(int offset, int regSize, boolean isStatic, Method method,
                                 Map<Integer, DexLabel> labelMap, DexDebugVisitor dcv, int maxOffset) {
     ...
     * {@link dex_reader_api.visitors.DexFileVisitor#visitEnd()} is not called
     *
     * @param dv       visitor
    ...
    private void findTryCatch(ByteBuffer in, DexCodeVisitor dcv, int tries_size, int insn_size,
        ...
        for (int i = 0; i < tries_size; i++) { // try_item
            ...
            ((Buffer) handlerIn).position(encoded_catch_handler_list + handler_offset);// move to encoded_catch_handler
    ...
    /* package */void acceptCode(int code_off, DexCodeVisitor dcv, int config, boolean isStatic, Method method) {
        ...
        if (debug_info_off != 0 && (0 == (config & SKIP_DEBUG))) {
            ...
            if (ddv != null) {
                try {
                    read_debug_info(debug_info_off, registers_size, isStatic, method, labelsMap, ddv, insnsArray.length);
                    ddv.visitEnd();
                } catch (Exception e) {
            ...
            switch (op.format) {
                ...
                case kFmt3rc: {
                    ...
                    if (op.indexType != InstructionIndexType.kIndexTypeRef
                            ...
                            || DvmFrame.descriptorLength(getMethod(b).getProto().getParameterTypes()) + (op == Op.INVOKE_STATIC
                            || op == Op.INVOKE_STATIC_RANGE
                            || op == Op.INVOKE_CUSTOM
                            || op == Op.INVOKE_CUSTOM_RANGE
                            ? 0
                            : 1) != 0)) // check whether the argument list is not empty
                    {
