...
/**
 * This sample application illustrates how to modify bytecode with the ProGuardCORE API. It adds
 * logging code at the start of all methods of all classes that it processes.
 *
 * <p>Usage: java proguard.examples.AddMethodInvocationLogging input.jar output.jar
 */
public class AddMethodInvocationLogging {
  public static void main(String[] args) throws IOException {
    ...
    JarWriter jarWriter =
        new JarWriter(new ZipWriter(new FixedFileWriter(new File(outputJarFileName))));

    // Parse and push all classes from the input jar.
    DataEntrySource source = new FileSource(new File(inputJarFileName));
            ...
            new ClassFilter(
                new ClassReader(
                    ...
                    new MultiClassVisitor(
                        // Modify the class.
                        new AllMethodVisitor(new AllAttributeVisitor(new MyLoggingAdder())),
  ...
  /**
   * This AttributeVisitor inserts logging instructions at the start of every code attribute that it
   * visits.
   */
  private static class MyLoggingAdder implements AttributeVisitor {
