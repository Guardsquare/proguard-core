...
/**
 * This sample application illustrates how to modify bytecode with the ProGuardCORE API. It applies
 * peephole optimizations to all code that it processes.
 *
 * <p>Usage: java proguard.examples.ApplyPeepholeOptimizations input.jar output.jar
 */
public class ApplyPeepholeOptimizations {
  ...
  public static void main(String[] args) {
    ...
    Instruction[][][] replacements = {
      { // ... + 0 = ...
        ____.iconst_0().iadd().__(),
      },
      { // i=i = nothing
        ____.iload(X).istore(X).__(),
      },
      { // putstatic/getstatic = dup/putstatic
        ____.putstatic(X).getstatic(X).__(), ____.dup().putstatic(X).__()
      },
      { // new StringBuffer().append(I) = new StringBuffer("....")
        ____.invokespecial(STRING_BUFFER, "<init>", "()V")
            ...
            .__(),
        ____.ldc_(INT_A_STRING).invokespecial(STRING_BUFFER, "<init>", "(Ljava/lang/String;)V").__()
      },
    };
