...
public class JarUtil {
   ...
   * @param isLibrary specifies whether classes should be represented as ProgramClass instances (for
   *     processing) or LibraryClass instances (more compact).
   ...
   * @return a new class pool with the read classes.
   */
  public static ClassPool readJar(String jarFileName, String classNameFilter, boolean isLibrary)
      throws IOException {
    ...
    DataEntryReader classReader =
        new NameFilteredDataEntryReader(
            ...
            new ClassReader(
                ...
                false,
                null,
                new ClassNameFilter(classNameFilter, classPoolFiller)));

    // Convert dex files to a JAR first.
    classReader =
        new NameFilteredDataEntryReader(
            "classes*.dex", new DexClassReader(!isLibrary, classPoolFiller), classReader);
    ...
    classReader =
        new FilteredDataEntryReader(
            ...
            new JarReader(
                new NameFilteredDataEntryReader("classes.jar", new JarReader(classReader))),
            new FilteredDataEntryReader(
   ...
   */
  public static void writeJar(ClassPool programClassPool, String outputJarFileName)
      throws IOException {
    JarWriter jarWriter =
