...
public interface JvmHeapAbstractState<StateT extends LatticeAbstractState<StateT>>
  ...
  /**
   * Creates a new object of a given {@link Clazz} at a specific program point and returns a
   * reference to it.
   */
  ...
  /**
   * Returns a field {@code fqn} from a reference {@code object}. If there is no abstract state
   * representing the field, returns the {@code defaultValue}
   */
  ...
  /**
   * Returns an {@code array} element at the specified {@code index} or the {@code defaultValue}, if
   ...
   *
   * <p>This can be overridden to model discarding heap portions at call sites.
   *
   * @param references information on the references to keep or discard, based on the
   *     implementation. Unused in the default implementation
   */
  default void reduce(Set<Object> references) {}

  /**
   * Expands the heap with references present in another state. Does nothing in the default
   * implementation.
   *
   * <p>This can be overridden to model recovering information discarded at call sites when
   * analyzing a return site.
   *
   * @param otherState a heap state from which expanding the heap (e.g. the state calling a method
