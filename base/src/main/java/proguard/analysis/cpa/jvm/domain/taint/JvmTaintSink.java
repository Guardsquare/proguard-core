...
/**
 * The {@link JvmTaintSink} adds an interface for extracting sensitive JVM memory locations and to
...
public abstract class JvmTaintSink extends TaintSink {
   ...
   * sink with the memory locations which triggers it. For convenience the sinks are further grouped
   * by their method signature.
   */
  public static Map<Signature, Map<JvmTaintSink, Set<JvmMemoryLocation>>>
      convertSinksToMemoryLocations(Collection<? extends JvmTaintSink> taintSinks) {
    ...
    for (JvmTaintSink taintSink : taintSinks) {
      ...
      result
          .computeIfAbsent(taintSink.signature, x -> new HashMap<>())
          .put(taintSink, memoryLocations);
    }
    return result;
  }
