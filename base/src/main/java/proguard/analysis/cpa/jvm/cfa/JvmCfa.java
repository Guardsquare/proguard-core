 ...
 *
 * <p>The nodes of a function are identified by the offset of the code location. Besides the normal
 * exit node for return instructions, function can also have a special exit node for uncaught
 * exceptions.
 *
...
public class JvmCfa extends Cfa<JvmCfaNode, JvmCfaEdge, MethodSignature> {
  ...
  private final Map<MethodSignature, Map<Integer, JvmCatchCfaNode>> functionCatchNodes =
      new HashMap<>();
  ...
  public JvmCatchCfaNode getFunctionCatchNode(MethodSignature signature, int offset) {
    return functionCatchNodes
        .getOrDefault(signature, Collections.emptyMap())
        .getOrDefault(offset, null);
  }
  ...
  public boolean containsFunctionCatchNode(MethodSignature signature, int offset) {
    return functionCatchNodes.containsKey(signature)
        && functionCatchNodes.get(signature).containsKey(offset);
  }
  ...
  /**
   * Returns the exception exit node (i.e. the exit node in case of not caught exception) of the
   * specified method if present, otherwise creates the exit node for the method and returns it.
   */
  ...
  /**
   * If the requested function node is present in the graph return it. If the node is not present
   * add it to the graph and return the new node.
   */
  public JvmCfaNode addNodeIfAbsent(MethodSignature signature, int offset, Clazz clazz) {
    // if the location at the current offset is already in the CFA (e.g. created by a branch or
    // goto) retrieve it, otherwise create a new node
  ...
  public void addInterproceduralEdge(Call call) {
    JvmCfaNode callNode =
        addNodeIfAbsent(
            ((MethodSignature) call.caller.signature), call.caller.offset, call.caller.clazz);
    JvmCfaNode calledNode =
        addNodeIfAbsent(
            call.getTarget(),
            0,
            call instanceof ConcreteCall ? ((ConcreteCall) call).getTargetClass() : null);

    new JvmCallCfaEdge(callNode, calledNode, call);
  }

  /**
   * Adds a call node between two methods. This is used when the target method has no {@link
   * proguard.classfile.attribute.CodeAttribute}, which means the target method is not present in
   * the CFA. The unknown node is set as the call target.
   */
  ...
  /**
   * Removes references to this CFA nodes from the singleton {@link JvmUnknownCfaNode} and clears
