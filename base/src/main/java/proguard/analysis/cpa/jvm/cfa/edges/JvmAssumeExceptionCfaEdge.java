...
public class JvmAssumeExceptionCfaEdge extends JvmCfaEdge {
   ...
   * @param catchType an integer representing the exception type in the edge's method exception
   *     table
   */
  ...
  /**
   * Create a JVM CFA exception assumption edge. Also sets it as the entering and leaving edge of
   ...
   *     table
   */
  public JvmAssumeExceptionCfaEdge(
      JvmCfaNode source, JvmCfaNode target, boolean isCaught, int catchType) {
    super(source, target);
    this.isCaught = isCaught;
    this.catchType = catchType;
  }
