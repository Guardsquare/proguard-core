 ...
 * proguard.analysis.cpa.jvm.state.heap.tree.JvmTreeHeapPrincipalAbstractState} and a sequence of
 * {@link JvmAbstractState}s which may have {@link JvmTreeHeapFollowerAbstractState}s depending on
 * the first abstract state. Join and copy are done elementwise preserving the link between the heap
 * models.
 *
 * <p>The composite abstract state must have the {@link JvmReferenceAbstractState} at its {@link
 * #REFERENCE_STATE_INDEX}th position. This abstract state will be used as the principal heap model
 * for other abstract states.
 *
 * @author Dmitry Ivanov
 */
public class CompositeHeapJvmAbstractState extends AbstractWrapperState
  ...
  // the position of the {@link JvmReferenceAbstractState} among the wrapped {@link
  // JvmAbstractState}
  public static final int REFERENCE_STATE_INDEX = 0;
  private final List<JvmAbstractState<? extends LatticeAbstractState<? extends AbstractState>>>
      jvmAbstractStates;

  /**
   * Create a composite abstract state from a list of JVM abstract states.
   *
   * @param jvmAbstractStates a list of {@link JvmAbstractState}s, must contain a {@link
   *     JvmReferenceAbstractState} at its {@code REFERENCE_INDEX}th position
   */
  public CompositeHeapJvmAbstractState(
  ...
  public JvmAbstractState<? extends LatticeAbstractState<? extends AbstractState>> getStateByIndex(
      int index) {
  ...
  public void updateHeapDependence() {
    jvmAbstractStates.stream()
        ...
        .forEach(
            h ->
                ((JvmTreeHeapFollowerAbstractState) h)
                    .setPrincipalState(
                        (JvmReferenceAbstractState)
                            jvmAbstractStates.get(
                                CompositeHeapJvmAbstractState.REFERENCE_STATE_INDEX)));
  }

  // implementations for AbstractWrapperState

  @Override
  public List<JvmAbstractState<? extends LatticeAbstractState<? extends AbstractState>>>
      getWrappedStates() {
  ...
  public CompositeHeapJvmAbstractState join(CompositeHeapJvmAbstractState abstractState) {
    ...
    return equals(result) ? this : abstractState.equals(result) ? abstractState : result;
  }
  ...
  public AbstractState copy() {
    List<JvmAbstractState<? extends LatticeAbstractState<? extends AbstractState>>>
        newJvmAbstractStates =
            jvmAbstractStates.stream().map(JvmAbstractState::copy).collect(Collectors.toList());
