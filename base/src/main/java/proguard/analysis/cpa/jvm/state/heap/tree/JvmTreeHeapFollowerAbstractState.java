 ...
 * does not track references to these objects though, it relies on {@link
 * JvmTreeHeapPrincipalAbstractState} instead for finding out which field/variable references an
 * object.
 *
...
public class JvmTreeHeapFollowerAbstractState<StateT extends LatticeAbstractState<StateT>>
  ...
  public JvmTreeHeapFollowerAbstractState(
      JvmReferenceAbstractState principal,
      StateT defaultValue,
  ...
  public <T> StateT getFieldOrDefault(T object, String fqn, StateT defaultValue) {
    if (object instanceof JvmMemoryLocation) {
      return getField(getReferenceAbstractState((JvmMemoryLocation) object), fqn, defaultValue);
    }
  ...
  public <T> void setField(T object, String fqn, StateT value) {
    if (object instanceof JvmMemoryLocation) {
      assignField(getReferenceAbstractState((JvmMemoryLocation) object), fqn, value);
      return;
    }
    if (object instanceof SetAbstractState) {
  ...
  public <T> StateT getArrayElementOrDefault(T array, StateT index, StateT defaultValue) {
    if (array instanceof JvmMemoryLocation) {
      return getArrayElementOrDefault(
          getReferenceAbstractState((JvmMemoryLocation) array), index, defaultValue);
    }
  ...
  public <T> void setArrayElement(T array, StateT index, StateT value) {
    if (array instanceof JvmMemoryLocation) {
      setArrayElement(getReferenceAbstractState((JvmMemoryLocation) array), index, value);
      return;
    }
    if (array instanceof SetAbstractState) {
  ...
  public void reduce(Set<Object> references) {
    Set<Reference> toKeep =
        ((JvmTreeHeapPrincipalAbstractState) principal.getHeap()).referenceToObject.keySet();
  ...
  public JvmTreeHeapFollowerAbstractState<StateT> join(JvmHeapAbstractState<StateT> abstractState) {
    JvmTreeHeapFollowerAbstractState<StateT> other =
        (JvmTreeHeapFollowerAbstractState<StateT>) abstractState;
    MapAbstractState<Reference, HeapNode<StateT>> newReferenceToNode =
    ...
    return new JvmTreeHeapFollowerAbstractState<>(
        principal,
        defaultValue,
        newReferenceToNode,
        heapMapAbstractStateFactory,
  ...
  public SetAbstractState<Reference> getReferenceAbstractState(
      JvmMemoryLocation principalMemoryLocation) {
    return principalMemoryLocation.extractValueOrDefault(principal, SetAbstractState.bottom);
  }
}
