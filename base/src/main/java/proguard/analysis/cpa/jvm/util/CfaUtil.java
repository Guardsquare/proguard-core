...
public class CfaUtil {
  ...
  /**
   * Returns a CFA for the given program class pool. Allows to limit the number of processed code
   * attributes with {@code shouldAnalyzeNextCodeAttribute}.
   *
   * @param programClassPool a program class pool
   */
  @Deprecated
  public static JvmCfa createIntraproceduralCfaFromClassPool(
      ClassPool programClassPool, Supplier<Boolean> shouldAnalyzeNextCodeAttribute) {
   ...
   * @param programClassPool a program class pool
   */
  public static JvmCfa createIntraproceduralCfa(
      ClassPool programClassPool, Supplier<Boolean> shouldAnalyzeNextCodeAttribute) {
  ...
  public static void addInterproceduralEdgesToCfa(JvmCfa cfa, CallGraph callGraph) {
    callGraph.outgoing.values().stream()
        .flatMap(Collection::stream)
        .filter(
            call ->
                !call.hasIncompleteTarget()
                    && cfa.getFunctionNode(
                            (MethodSignature) call.caller.signature, call.caller.offset)
                        != null)
        .forEach(
            call -> {
              if (call instanceof SymbolicCall
                  ...
                  || ((ConcreteCall) call).getTargetMethod() instanceof ProgramMethod
                      && Arrays.stream(
                              ((ProgramMethod) ((ConcreteCall) call).getTargetMethod()).attributes)
   ...
   */
  @Deprecated
  public static JvmCfa createInterproceduralCfaFromClassPoolAndCallGraph(
      ClassPool programClassPool, CallGraph callGraph) {
   ...
   */
  @Deprecated
  public static JvmCfa createInterproceduralCfaFromClassPool(
   ...
   * @param programClassPool a program class pool
   */
  public static JvmCfa createInterproceduralCfa(
      ClassPool programClassPool, ClassPool libraryClassPool) {
    CallGraph callGraph = new CallGraph();
    CallResolver resolver =
        new CallResolver.Builder(programClassPool, libraryClassPool, callGraph)
                      ...
                      edge -> {
                        ...
                        if (edge instanceof JvmInstructionCfaEdge) {
                          ...
                          sb.append(
                              instruction
                                  .toString(
                                      edge.getTarget().getClazz(), edge.getSource().getOffset())
                                  .replace("\"", "\\\""));
                          if (edge instanceof JvmAssumeCfaEdge) {
                            sb.append("\n(")
                                .append(((JvmAssumeCfaEdge) edge).isSatisfied())
                                .append(")");
                          }
