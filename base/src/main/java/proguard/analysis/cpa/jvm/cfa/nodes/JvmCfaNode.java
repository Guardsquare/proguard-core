...
public class JvmCfaNode implements CfaNode<JvmCfaEdge, MethodSignature> {
  ...
  /**
   * Create a JVM CFA node without edges. Since in most cases we expect to have just one element in
   * the lists of leaving and entering edges the lists are initialized with size 1.
   *
  ...
  // each node is identified by a hash calculated from the method signature, and the offset, and the
  // class (e.g. to distinguish the catch nodes from the ones with same offset)
  @Override
  public int hashCode() {
  ...
  /**
   * If the node is the location after a method invocation, returns the entering {@link
  ...
  public Optional<JvmCfaEdge> getEnteringInvokeEdge() {
    return enteringEdges.stream()
        .filter(
            e ->
                e instanceof JvmInstructionCfaEdge
                    && InstructionClassifier.isInvoke(
                        ((JvmInstructionCfaEdge) e).getInstruction().opcode))
        .findFirst();
  }

  /**
   * If the node is the location before a method invocation, returns the leaving {@link
   * JvmInstructionCfaEdge} for the method invocation, empty otherwise.
   */
