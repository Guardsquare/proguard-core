...
/**
 * A {@link JvmAbstractState} for the reference CPA. Since the analysis may skip library methods
 ...
 * can be missed and the analysis result becomes unsound.
 *
...
public class JvmReferenceAbstractState extends JvmAbstractState<SetAbstractState<Reference>> {
   ...
   */
  public JvmReferenceAbstractState(
  ...
  @Override
  public SetAbstractState<Reference> getVariableOrDefault(
      int index, SetAbstractState<Reference> defaultState) {
    if (index < frame.getLocalVariables().size()) {
      return super.getVariableOrDefault(index, defaultState);
    }
    return new SetAbstractState<>(
        new Reference(getMethodEntryNode(), new JvmLocalVariableLocation(index)));
  }

  @Override
  public SetAbstractState<Reference> getStaticOrDefault(
      String fqn, SetAbstractState<Reference> defaultState) {
    if (staticFields.containsKey(fqn)) {
      return super.getStaticOrDefault(fqn, defaultState);
    }
    return new SetAbstractState<>(
        new Reference(JvmUnknownCfaNode.INSTANCE, new JvmStaticFieldLocation(fqn)));
  }

  @Override
  public <T> SetAbstractState<Reference> getFieldOrDefault(
      T object, String descriptor, SetAbstractState<Reference> defaultValue) {
  ...
  @Override
  public JvmReferenceAbstractState join(
      JvmAbstractState<SetAbstractState<Reference>> abstractState) {
    JvmReferenceAbstractState answer =
  ...
  public JvmReferenceAbstractState copy() {
    return new JvmReferenceAbstractState(
  ...
  private JvmCfaNode getMethodEntryNode() {
    ...
    while (result.getOffset() != 0) {
      result =
          result.getEnteringEdges().stream()
              .map(JvmCfaEdge::getSource)
              .min(Comparator.comparingInt(JvmCfaNode::getOffset))
              .get();
    }
    return result;
  }
