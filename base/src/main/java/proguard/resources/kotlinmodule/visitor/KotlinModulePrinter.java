...
public class KotlinModulePrinter
    implements ResourceFileVisitor, KotlinModuleVisitor, KotlinModulePackageVisitor {
  ...
  public void visitKotlinModulePackage(
      KotlinModule kotlinModule, KotlinModulePackage kotlinModulePart) {
    ...
    for (int i = 0; i < kotlinModulePart.fileFacadeNames.size(); i++) {
      ...
      println(
          "[FFAC] "
              + hasRefIndicator(kotlinModulePart.referencedFileFacades.get(i))
              + fileFacadeName);
    }

    kotlinModulePart.multiFileClassParts.values().stream()
        .distinct()
        .forEachOrdered(
            (multiFileFacadeName) -> {
              ...
              indent();
              kotlinModulePart.multiFileClassParts.entrySet().stream()
                  ...
                  .forEachOrdered(
                      multiFilePartName ->
                          println(
                              "[MPRT] "
                                  + hasRefIndicator(
                                      kotlinModulePart.referencedMultiFileParts.get(
                                          multiFilePartName))
                                  + multiFilePartName));
              outdent();
            });
