...
/**
 * This ClassVisitor collects all lambda expressions that are defined in a visited class. Such
 * functionality is useful when you would like to resolve the actual method that is called when
 * invokedynamic wants to execute a lambda expression.
 *
 * @author Thomas Neidhart
 */
public class LambdaExpressionCollector
    implements ClassVisitor, ConstantVisitor, AttributeVisitor, BootstrapMethodInfoVisitor {
  ...
  public void visitProgramClass(ProgramClass programClass) {
    ...
    programClass.constantPoolEntriesAccept(new ConstantTagFilter(Constant.INVOKE_DYNAMIC, this));
  }
  ...
  public void visitBootstrapMethodsAttribute(
    ...
    bootstrapMethodsAttribute.bootstrapMethodEntryAccept(
        clazz, referencedBootstrapMethodIndex, this);
  }
  ...
  public void visitBootstrapMethodInfo(Clazz clazz, BootstrapMethodInfo bootstrapMethodInfo) {
    ...
    if (isLambdaMetaFactory(bootstrapMethodHandle.getClassName(clazz))) {
      ...
      String interfaceClassName =
          ClassUtil.internalClassNameFromClassType(
              ClassUtil.internalMethodReturnType(factoryMethodDescriptor));
      ...
      LambdaExpression lambdaExpression =
          new LambdaExpression(
              ...
              getMethodTypeConstant(programClass, bootstrapMethodInfo.u2methodArguments[0])
                  .getType(clazz),
      ...
      if (isAlternateFactoryMethod(bootstrapMethodHandle.getName(clazz))) {
        ...
        if ((flags & BootstrapMethodInfo.FLAG_MARKERS) != 0) {
          int markerInterfaceCount =
              getIntegerConstant(
                  programClass, bootstrapMethodInfo.u2methodArguments[argumentIndex++]);
          ...
          for (int i = 0; i < markerInterfaceCount; i++) {
            ...
            lambdaExpression.interfaces =
                ArrayUtil.add(
                    lambdaExpression.interfaces, lambdaExpression.interfaces.length, interfaceName);
          }
        }
  ...
  private static final String NAME_JAVA_LANG_INVOKE_LAMBDA_METAFACTORY =
  ...
  private static MethodTypeConstant getMethodTypeConstant(
      ProgramClass programClass, int constantIndex) {
