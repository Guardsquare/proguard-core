...
public class DebugAttributeVisitor implements AttributeVisitor {
   ...
   * Creates a new DebugAttributeVisitor.
   *
   * @param message the message to be printed when the maximum invocation time is exceeded.
   * @param attributeVisitor the AttributeVisitor to which visits will be delegated.
   */
  public DebugAttributeVisitor(String message, AttributeVisitor attributeVisitor) {
    this(message, 10000L, attributeVisitor);
  }
  ...
  public DebugAttributeVisitor(
      String message, long maximumTime, AttributeVisitor attributeVisitor) {
  ...
  /**
   * Checks the time after having visited the given attribute, and prints out a message if
   * necessary.
   */
  private void checkTime(Clazz clazz, Attribute attribute, long startTime) {
    if (DEBUG) {
      ...
      if (deltaTime > maximumTime) {
        System.err.println(
            "=== " + message + " took " + ((double) deltaTime / 1000.) + " seconds ===");
   ...
   * necessary.
   */
  private void checkTime(Clazz clazz, Field field, Attribute attribute, long startTime) {
    if (DEBUG) {
      long endTime = startTime();
   ...
   * necessary.
   */
  private void checkTime(Clazz clazz, Method method, Attribute attribute, long startTime) {
    if (DEBUG) {
      long endTime = startTime();
