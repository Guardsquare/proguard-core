...
/**
 * This {@link ClassVisitor} removes specified interfaces from the classes and class signatures that
 * it visits.
 *
 * @author Eric Lafortune
 */
public class InterfaceDeleter implements ClassVisitor, AttributeVisitor {
  /*
   ...
   *
   * @param delete an array that corresponds to the interfaces of a class and that specifies the
   *     ones to be removed.
   * @param removeSubclasses specifies whether to remove the specified interfaces as subclasses of
   *     their superclasses.
   */
  ...
  public void visitProgramClass(ProgramClass programClass) {
    ...
    if (DEBUG) {
      System.out.println(
          "InterfaceDeleter: " + programClass.getName() + " (" + interfacesCount + " interfaces)");
    }
    ...
    for (int index = 0; index < interfacesCount; index++) {
      if (DEBUG) {
        System.out.println(
            "InterfaceDeleter:   "
                + (delete[index] ? "- " : "+ ")
                + programClass.getInterfaceName(index));
      }
      ...
      if (!delete[index]) {
        ...
        programClass.constantPoolEntryAccept(
            interfaceIndex, new ReferencedClassVisitor(new SubclassRemover(programClass)));
      }
    }
  ...
  public void visitSignatureAttribute(Clazz clazz, SignatureAttribute signatureAttribute) {
    ...
    if (referencedClasses != null) {
      ...
      while (internalTypeEnumeration.hasMoreTypes()) {
        ...
        if (DEBUG) {
          System.out.println(
              "InterfaceDeleter:   interface type "
                  + (delete[index] ? "- " : "+ ")
                  + type
                  + " ("
                  + classCount
