...
/**
 * This {@link AttributeVisitor} accumulates instructions, exceptions and line numbers, and then
 * adds them to a method or copies them into code attributes that it visits.
 *
 * <p>The class supports composing instructions ({@link #appendInstruction(Instruction)}), labels
 * ({@link #appendLabel(int)}), exception handlers ({@link #appendException(ExceptionInfo)}), and
 * line numbers ({@link #appendLineNumber(LineNumberInfo)}).
 *
 * <p>The labels are numeric labels that you can choose freely, for example instruction offsets from
 ...
 *
 * <p>You should provide an estimated maximum size (expressed in number of bytes in the bytecode),
 ...
 * </pre>
 *
 * <p>This class is mostly convenient to compose code based on existing code, where the instructions
 * are already available. For a more compact and readable alternative to compose code
 * programmatically from scratch, see {@link CompactCodeAttributeComposer}.
 *
 * <p>If you're building many method bodies, it is more efficient to reuse a single instance of this
...
public class CodeAttributeComposer
  ...
  private ExceptionInfo[] exceptionTable =
      new ExceptionInfo[ClassEstimates.TYPICAL_EXCEPTION_TABLE_LENGTH];
  private LineNumberInfo[] lineNumberTable =
      new LineNumberInfo[ClassEstimates.TYPICAL_LINE_NUMBER_TABLE_LENGTH];
   ...
   * @param shrinkInstructions specifies whether instructions should automatically be shrunk before
   *     being written.
   * @param absoluteBranchOffsets specifies whether offsets of appended branch instructions and
  ...
  public void beginCodeFragment(int maximumCodeFragmentLength) {
    ...
    if (level >= MAXIMUM_LEVELS) {
      throw new IllegalArgumentException(
          "Maximum number of code fragment levels exceeded [" + level + "]");
    }
  ...
  /**
   * Appends the given instruction with the given old offset. Branch instructions must fit, for
   * instance by enabling automatic shrinking of instructions.
   *
   * @param oldInstructionOffset the old offset of the instruction, to which branches and other
   *     references in the current code fragment are pointing.
   ...
   *
   * @param oldInstructionOffset the old offset of the label, to which branches and other references
   *     in the current code fragment are pointing.
   */
  ...
  /**
   * Appends the given instruction without a defined offset. Branch instructions should have a
   * label, to allow computing the new relative offset. Branch instructions must fit, for instance
   * by enabling automatic shrinking of instructions.
   *
   * @param instruction the instruction to be appended.
   */
  ...
  public void appendLineNumber(LineNumberInfo lineNumberInfo) {
    ...
    // Add the line number.
    lineNumberTable = ArrayUtil.add(lineNumberTable, lineNumberTableLength++, lineNumberInfo);
  }
  ...
  public void endCodeFragment() {
    ...
    maximumCodeLength += codeLength - codeFragmentOffsets[level] - codeFragmentLengths[level];
    ...
    if (allowExternalExceptionOffsets) {
      for (int index = 0; index < exceptionTableLength; index++) {
        ...
        exceptionInfo.u2startPC = remapExceptionOffset(exceptionInfo.u2startPC);
        exceptionInfo.u2endPC = remapExceptionOffset(exceptionInfo.u2endPC);
        exceptionInfo.u2handlerPC = remapExceptionOffset(exceptionInfo.u2handlerPC);
      }
    }
  ...
  public void visitCodeAttribute(Clazz clazz, Method method, CodeAttribute codeAttribute) {
    ...
    if (lineNumberTableLength > 0
      ...
      int attributeNameIndex = constantPoolEditor.addUtf8Constant(Attribute.LINE_NUMBER_TABLE);
  ...
  public void visitLineNumberTableAttribute(
    ...
    if (lineNumberTableLength == 0) {
      ...
      System.arraycopy(
          lineNumberTable, 0, lineNumberTableAttribute.lineNumberTable, 0, lineNumberTableLength);
    }
  }
  ...
  public void visitBranchInstruction(
    ...
    try {
      ...
      branchInstruction.branchOffset = newBranchOffset(offset, branchInstruction.branchOffset);
  ...
  public void visitAnySwitchInstruction(
    ...
    try {
      ...
      switchInstruction.defaultOffset = newBranchOffset(offset, switchInstruction.defaultOffset);

      // Adjust the jump offsets.
      updateJumpOffsets(offset, switchInstruction.jumpOffsets);
  ...
  public void visitExceptionInfo(
    ...
    exceptionInfo.u2startPC =
        !allowExternalExceptionOffsets || remappableExceptionOffset(startPC)
    ...
    exceptionInfo.u2endPC =
        !allowExternalExceptionOffsets || remappableExceptionOffset(endPC)
    ...
    exceptionInfo.u2handlerPC =
        !allowExternalExceptionOffsets || remappableExceptionOffset(handlerPC)
  ...
  private int newBranchOffset(int newInstructionOffset, int oldBranchOffset) {
    ...
    int oldInstructionOffset =
        absoluteBranchOffsets ? 0 : oldInstructionOffsets[newInstructionOffset];

    // Compute the new branch offset, always relative to the instruction.
    return newInstructionOffset(oldInstructionOffset + oldBranchOffset) - newInstructionOffset;
  }

  /** Computes the new instruction offset for the instruction at the given old offset. */
  private int newInstructionOffset(int oldInstructionOffset) {
    if (oldInstructionOffset < 0 || oldInstructionOffset > codeFragmentLengths[level]) {
      throw new IllegalArgumentException(
          "Instruction offset ["
              + oldInstructionOffset
              + "] out of range in code fragment with length ["
  ...
  /**
   * Computes the new instruction offset for an exception start, end, or handler, if the old
   * instruction offset is negated.
   */
    ...
    if (oldInstructionOffset < 0) {
      ...
      if (remappableExceptionOffset(oldInstructionOffset)) {
        ...
        throw new IllegalStateException(
            "Couldn't remap exception offset [" + oldInstructionOffset + "]");
      }
    }
  ...
  /**
   * Returns whether the given old exception offset can be remapped in the current code fragment.
   */
  ...
  /**
   * Returns the given list of line numbers, without the ones that have empty code blocks or that
   * exceed the code size.
   */
  private int removeEmptyLineNumbers(
