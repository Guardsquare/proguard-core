...
/**
 * This editor allows to build or extend classes (ProgramClass instances). It provides methods to
 * easily add interfaces, fields, and methods, optionally with method bodies.
 *
 * <p>If you're adding many fields and methods, it is more efficient to reuse a single instance of
 * this builder for all fields and methods that you add.
 *
 * @author Johan Leys
...
public class ClassBuilder {
  ...
  public ClassBuilder(
    ...
    programClass.u2thisClass = constantPoolEditor.addClassConstant(className, programClass);

    if (superclassName != null) {
      programClass.u2superClass = constantPoolEditor.addClassConstant(superclassName, null);
    }
  }
  ...
  /**
   * Creates a new ClassBuilder for the given class, that automatically initializes class references
   * and class member references in new constants.
   *
   * @param programClass the class to be edited.
   ...
   */
  public ClassBuilder(
      ProgramClass programClass, ClassPool programClassPool, ClassPool libraryClassPool) {

    this.programClass = programClass;

    classEditor = new ClassEditor(programClass);
    constantPoolEditor = new ConstantPoolEditor(programClass, programClassPool, libraryClassPool);
    compactCodeAttributeComposer =
        new CompactCodeAttributeComposer(
            constantPoolEditor, new CodeAttributeComposer(false, true, true));
  }

  /**
   * Returns the created or edited ProgramClass instance. This is a live instance; any later calls
   * to the builder will still affect the instance.
   */
   ...
   * instance is more efficient for newly created classes.
   */
  ...
  public ClassBuilder addInterface(String interfaceName, Clazz referencedInterface) {
    // Add it to the class.
    classEditor.addInterface(
        constantPoolEditor.addClassConstant(interfaceName, referencedInterface));
  ...
  public ClassBuilder addField(int u2accessFlags, String fieldName, String fieldDescriptor) {
    return addField(u2accessFlags, fieldName, fieldDescriptor, null);
  }
  ...
  public ClassBuilder addField(
    ...
    // Create the field.
    ProgramField programField = addAndReturnField(u2accessFlags, fieldName, fieldDescriptor);
  ...
  public ClassBuilder addMethod(int u2accessFlags, String methodName, String methodDescriptor) {
    return addMethod(u2accessFlags, methodName, methodDescriptor, null);
  }
   ...
   * @param methodName the name of the new method.
   * @param methodDescriptor the descriptor of the new method.
   * @param extraMemberVisitor an optional visitor for the method after it has been created and
   *     added to the class.
   * @return this instance of ClassBuilder.
   */
  public ClassBuilder addMethod(
      ...
      MemberVisitor extraMemberVisitor) {
    // Create the method.
    ProgramMethod programMethod = addAndReturnMethod(u2accessFlags, methodName, methodDescriptor);
   ...
   */
  public ProgramMethod addAndReturnMethod(
      int u2accessFlags, String methodName, String methodDescriptor) {
    return addAndReturnMethod(u2accessFlags, methodName, methodDescriptor, 0, null);
  }
   ...
   * @param codeBuilder the provider of a composer to create code attributes.
   * @return this instance of ClassBuilder.
   */
  public ClassBuilder addMethod(
      int u2accessFlags,
    ...
    return addMethod(
        u2accessFlags, methodName, methodDescriptor, maxCodeFragmentLength, codeBuilder, null);
  }
  ...
  public ClassBuilder addMethod(
    ...
    ProgramMethod programMethod =
        addAndReturnMethod(
            u2accessFlags, methodName, methodDescriptor, maxCodeFragmentLength, codeBuilder);
