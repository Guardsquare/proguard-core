 ...
 *
 * <p>If all bootstrap methods are removed, it also removes the {@link BootstrapMethodsAttribute}
 * from the visited class. Additionally, the <code>java/lang/MethodHandles$Lookup</code> class will
 * be removed from the {@link InnerClassesAttribute}, and the {@link InnerClassesAttribute} will be
 * removed if it was the only entry.
 *
...
public class BootstrapMethodsAttributeShrinker
  ...
  private int[] bootstrapMethodIndexMap =
      new int[ClassEstimates.TYPICAL_BOOTSTRAP_METHODS_ATTRIBUTE_SIZE];
  ...
  public void visitBootstrapMethodsAttribute(
    ...
    if (referencedBootstrapMethodIndex > -1) {
      ...
      // The bootstrap methods have been marked, so now we shrink the array of BootstrapMethodInfo
      // objects.
      int newBootstrapMethodsCount =
          shrinkBootstrapMethodArray(
  ...
  /**
   * Removes all entries that are not marked as being used from the given array of bootstrap
   * methods. Creates a map from the old indices to the new indices as a side effect.
   *
  ...
  private class MethodHandlesLookupInnerClassRemover
    ...
    public void visitInnerClassesInfo(Clazz clazz, InnerClassesInfo innerClassesInfo) {
      ...
      if (isMethodHandleClass(innerClass, clazz) || isMethodHandleClass(outerClass, clazz)) {
