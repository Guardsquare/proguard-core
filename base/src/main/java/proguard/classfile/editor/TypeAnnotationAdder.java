...
/**
 * This {@link TypeAnnotationVisitor} adds all type annotations that it visits to the given target
 * annotation element value, target annotation attribute, or target parameter annotation attribute.
 *
 * @author Eric Lafortune
 */
public class TypeAnnotationAdder implements TypeAnnotationVisitor {
  ...
  /**
   * Creates a new TypeAnnotationAdder that will copy annotations into the given target annotation
   * element value.
   */
  public TypeAnnotationAdder(
  ...
  /**
   * Creates a new TypeAnnotationAdder that will copy annotations into the given target annotations
   * attribute.
   */
  public TypeAnnotationAdder(
  ...
  public void visitTypeAnnotation(Clazz clazz, TypeAnnotation typeAnnotation) {
    ...
    TypeAnnotation newTypeAnnotation =
        new TypeAnnotation(
            constantAdder.addConstant(clazz, typeAnnotation.u2typeIndex),
            0,
    ...
    typeAnnotation.elementValuesAccept(
        clazz, new ElementValueAdder(targetClass, newTypeAnnotation, false));

    // Set the target info.
    typeAnnotation.targetInfo.accept(
        clazz, typeAnnotation, new TargetInfoCopier(targetClass, newTypeAnnotation));

    // Copy the type path.
    for (int index = 0; index < typePath.length; index++) {
      TypePathInfo typePathInfo = typePath[index];
      TypePathInfo newTypePathInfo =
          new TypePathInfo(typePathInfo.u1typePathKind, typePathInfo.u1typeArgumentIndex);
