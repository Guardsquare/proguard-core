...
/**
 * This utility class renders {@link Method} objects into a more human-readable format. It can be
...
public class MethodViewModel extends MemberViewModel {
  ...
  /**
   * A utility method that renders a {@link Method} entry into a preview string, e.g., private int
   * bar(Foo obj)
   *
  ...
  /**
   * A utility method for rendering the method's attributes (e.g., code attributes) into a
   * human-readable format, i.e., it prints out the byte code instead of showing just the byte
   * array.
   *
   * @param clazz The {@link Clazz} that the {@link Method} belongs to.
            ...
            programMethod.attributesAccept(
                ...
                new AttributeVisitor() {
                  ...
                  public void visitSignatureAttribute(
                      Clazz clazz, SignatureAttribute signatureAttribute) {
                    signatureAttributeList.add(
                        renderAttributeAsString(clazz, method, signatureAttribute));
                  ...
                  public void visitExceptionsAttribute(
                    ...
                    exceptionAttributeList.add(
                        renderAttributeAsString(clazz, method, exceptionsAttribute));
                  ...
                  public void visitRuntimeVisibleTypeAnnotationsAttribute(
                      ...
                      RuntimeVisibleTypeAnnotationsAttribute
                          runtimeVisibleTypeAnnotationsAttribute) {
                    runtimeInvisibleAnnotationAttributeList.add(
                        renderAttributeAsString(
                            clazz, method, runtimeVisibleTypeAnnotationsAttribute));
                    if (!attributeList.containsKey("runtime invisible annotations"))
                      attributeList.put(
                          "runtime invisible annotations", runtimeInvisibleAnnotationAttributeList);
                  }
                  ...
                  public void visitAnyAnnotationsAttribute(
                    ...
                    runtimeInvisibleAnnotationAttributeList.add(
                        renderAttributeAsString(clazz, method, annotationsAttribute));
                    if (!attributeList.containsKey(annotationsAttribute.getClass().getName()))
                      attributeList.put(
                          annotationsAttribute.getClass().getName(), attributeContainer);
                  }

                  private String renderAttributeAsString(
                      Clazz clazz, Method method, Attribute attribute) {
                    StringWriter stringWriter = new StringWriter();
                    attribute.accept(
                        clazz, method, new ClassPrinter(new PrintWriter(stringWriter)));
                    stringWriter.flush();
                    return stringWriter.toString();
                  }

                  private List<String> renderAttributeAsList(
                      Clazz clazz, Method method, Attribute attribute) {
