...
/**
 * This {@link ClassVisitor} replaces array initialization instructions with optimized primitive
 * array constants.
 *
 * <p>These constants are not supported by any Java specification and therefore only for internal
 * use.
 *
...
public class ArrayInitializationReplacer
  ...
  private final ValueFactory valueFactory =
      new ParticularValueFactory(new BasicValueFactory(), new BasicValueFactory());
  ...
  private final ArrayInitializationMatcher arrayInitializationMatcher =
      new ArrayInitializationMatcher(partialEvaluator);
  ...
  public void visitAnyInstruction(
    ...
    if (instruction.opcode == Instruction.OP_NEWARRAY && lastInstructionStackPushCount == 1) {
      if (arrayInitializationMatcher.matchesArrayInitialization(
          clazz, method, codeAttribute, offset, (SimpleInstruction) instruction)) {
        ...
        codeAttributeEditor.replaceInstruction(
            lastInstructionOffset, new ConstantInstruction(Instruction.OP_LDC, constantIndex));
  ...
  /** Private utility class to visit only CodeAttributes that contain "NEWARRAY" instructions. */
  private static class ArrayInitializationFilter implements AttributeVisitor {
