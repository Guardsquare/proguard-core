 ...
 *
 * <p>The leading formal type parameters, if any, can be retrieved separately.
 *
 * <p>The return type of a method descriptor can also be retrieved separately.
 *
...
public class InternalTypeEnumeration implements Iterator<String> {
  ...
  public InternalTypeEnumeration(String descriptor) {
    ...
    this.openIndex =
        descriptor.indexOf(TypeConstants.METHOD_ARGUMENTS_OPEN, formalTypeParametersIndex);
  ...
  /**
   * Returns the number of types contained in the descriptor. This is the number of types that the
   * enumeration will return.
   */
  ...
  /**
   * Returns the total size of the types contained in the descriptor. This accounts for long and
   * double parameters taking up two entries.
   */
    ...
    try {
      for (int index = 0; index < args.length; index++) {
        ...
        if (enumeration.hasFormalTypeParameters()) {
          System.out.println(
              "  Formal type parameters [" + enumeration.formalTypeParameters() + "]");
        }
