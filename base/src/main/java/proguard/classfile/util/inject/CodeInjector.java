...
/**
 * This utility class allow for injecting a method invocation instruction optionally with arguments modeled by instances
 * of classes implementing {@link InjectedArgument} interface to the specified target method at an offset determined by
 * the implementation of the {@link InjectStrategy} interface.
 *
 * Example usage:
 * new CodeInjector()
 *    .injectInvokeStatic(logUtilClass, logDebugMethod,
 *                        new ConstantPrimitive<Integer>(1), new ConstantString("Hello world"))
 *    .into(MainProgramClass, mainMethod)
 *    .at(new FirstBlock())
 *    .commit();
 *
 * @author Kymeng Tang
...
public class CodeInjector {
    ...
    public CodeInjector injectInvokeStatic(Clazz clazz, Method method) {
        assert content == null :
            "The injection content: `" + renderInjectionContent(content.clazz, content.method, arguments) + "` " +
    ...
    public CodeInjector injectInvokeStatic(Clazz clazz, Method method, InjectedArgument... arguments) {
        ...
        while (parametersIterator.hasNext() || argumentsIterator.hasNext()) {
            ...
            assert expectedType.equals(provided.getInternalType()) :
                String.format("Provided argument: `%s` doesn't match the expected parameter type: %s for method: ",
                        argumentsIterator, expectedType, renderMethodSignature(content.clazz, content.method));
        }
     ...
     * @param programClass The program class that has the method where a static method invocation shall be injected into.
     ...
     * @param injectStrategy The implementation of InjectStrategy interface which determine the offset to inject
    ...
    public void commit() {
        ...
        assert targets != null:
            ...
            "injecting " + renderInjectionContent(content.clazz, content.method, arguments) + ".";

        assert injectStrategy != null:
            "The injection location hasn't been provided. please use `.at(...)` to indicate the place to inject " +
            renderInjectionContent(content.clazz, content.method, arguments) + " into the target method.";
        ...
        targets.forEach(target -> {
            ...
            arguments.forEach(argument -> code.pushPrimitiveOrString(argument.getValue(), argument.getInternalType()));
            code.invokestatic(content.clazz, content.method);
            
            target.method.accept(target.clazz,
                ...
                new AttributeVisitor() {
                    ...
                    public void visitCodeAttribute(Clazz clazz, Method method, CodeAttribute codeAttribute) {
                        ...
                        InjectStrategy.InjectLocation[] injectLocations =
                                injectStrategy.getAllSuitableInjectionLocation((ProgramClass) clazz, (ProgramMethod) method);
                        for (InjectStrategy.InjectLocation location : injectLocations) {
                            final BiConsumer<Integer, Instruction[]> inserter = location.shouldInjectBefore()
