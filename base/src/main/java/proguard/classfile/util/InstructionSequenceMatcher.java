...
/**
 * This {@link InstructionVisitor} checks whether a given pattern instruction sequence occurs in the
 * instructions that are visited. The arguments of the instruction sequence can be wildcards that
 * are matched.
 *
 * @author Eric Lafortune
 */
public class InstructionSequenceMatcher implements InstructionVisitor, ConstantVisitor {
  // *
   ...
   *
   * @param patternConstants any constants referenced by the pattern instruction.
   * @param patternInstructions the pattern instruction sequence.
   */
  public InstructionSequenceMatcher(
      Constant[] patternConstants, Instruction[] patternInstructions) {
   ...
   * @param patternConstants any constants referenced by the pattern instruction.
   * @param patternInstructions the pattern instruction sequence.
   * @param matchSubclasses whether constant references on subclasses are matching when the pattern
   *     contains a constant of a superclass. (default: false)
   */
  public InstructionSequenceMatcher(
      Constant[] patternConstants, Instruction[] patternInstructions, boolean matchSubclasses) {
  ...
  public int matchedArgument(int argument) {
    ...
    return argumentIndex < 0 ? argument : matchedArguments[argumentIndex];
  }
  ...
  public int matchedBranchOffset(int offset, int branchOffset) {
    ...
    return argumentIndex < 0 ? branchOffset : matchedArguments[argumentIndex] - offset;
  }
  ...
  public int[] matchedJumpOffsets(int offset, int[] jumpOffsets) {
    ...
    for (int index = 0; index < jumpOffsets.length; index++) {
      matchedJumpOffsets[index] = matchedBranchOffset(offset, jumpOffsets[index]);
    }
  ...
  /** Marks the specified argument (by index) as matching the specified argument value. */
  ...
  /** Returns whether the specified wildcard argument (by index) has been matched. */
  ...
  /** Marks the specified constant (by index) as matching the specified constant index value. */
  ...
  protected boolean matchingJumpOffsets(int offset, int[] jumpOffsets1, int[] jumpOffsets2) {
    ...
    for (int index = 0; index < jumpOffsets1.length; index++) {
      if (!matchingBranchOffsets(offset, jumpOffsets1[index], jumpOffsets2[index])) {
        return false;
      }
    }
