 ...
 * use this information to initialize a {@link CallableReferenceInfo} inside the synthetic class.
 *
 * <p>FunctionReferences are lambda synthetic classes PropertyReferences are regular synthetic
 * classes LocalVariableReferences extend PropertyReferences
 *
...
public class KotlinCallableReferenceInitializer implements KotlinMetadataVisitor {
  ...
  public KotlinCallableReferenceInitializer(
      ClassPool programClassPool, ClassPool libraryClassPool) {
  ...
  public void visitKotlinSyntheticClassMetadata(
      Clazz clazz, KotlinSyntheticClassKindMetadata syntheticClassKindMetadata) {
    if (clazz.extendsOrImplements(REFLECTION.CALLABLE_REFERENCE_CLASS_NAME)) {
      Consumer<InfoLoaderResult> loader =
          result -> {
            ...
            if (clazz.extendsOrImplements(REFLECTION.FUNCTION_REFERENCE_CLASS_NAME)) {
              ...
              if (method != null) {
                ...
                if (result.callableOwnerMetadata != null) {
                  method.accept(
                      methodReferencedClass,
                      new MethodToKotlinFunctionVisitor(
                          new FunctionReferenceInfoInitializer(
                              result.callableOwnerClass,
                ...
                if (syntheticClassKindMetadata.callableReferenceInfo == null) {
                  ...
                  syntheticClassKindMetadata.callableReferenceInfo =
                      new JavaMethodReferenceInfo(
                          result.callableOwnerClass, methodReferencedClass, method);
                }
              }
              ...
              if (syntheticClassKindMetadata.callableReferenceInfo == null) {
                // If we couldn't find any in the Kotlin metadata, we can search for the field by
                // name/descriptor (assuming there is a backing field).
                ...
                if (field != null) {
                  syntheticClassKindMetadata.callableReferenceInfo =
                      new JavaFieldReferenceInfo(
                          result.callableOwnerClass, memberFinder.correspondingClass(), field);
                }
              }
            }
          };
  ...
  /**
   * Helper class to load the callable reference information from the existing implementations in a
   * class generated by kotlin 1.4+ compiler
   */
  private class CallableReferenceInfoLoader1dot4 implements ClassVisitor, InstructionVisitor {
    ...
    public CallableReferenceInfoLoader1dot4(Consumer<InfoLoaderResult> consumer) {
      ...
      builder.ldc_(OWNER_INDEX).ldc_(NAME_INDEX).ldc_(SIGNATURE_INDEX).iconst(I).invokespecial(X);
    ...
    public void visitAnyClass(Clazz clazz) {
      ...
      clazz.methodsAccept(
          new MemberNameFilter(
              METHOD_NAME_INIT, new AllAttributeVisitor(new AllInstructionVisitor(this))));
    }
      ...
      for (InstructionSequenceMatcher matcher : matchers) {
        ...
        if (matcher.isMatching()) {
          ...
          clazz.constantPoolEntryAccept(
              matcher.matchedConstantIndex(OWNER_INDEX),
              new ConstantVisitor() {
                @Override
          ...
          clazz.constantPoolEntryAccept(
              matcher.matchedConstantIndex(NAME_INDEX),
              new ConstantVisitor() {
                @Override
                public void visitAnyConstant(Clazz clazz, Constant constant) {}
          ...
          clazz.constantPoolEntryAccept(
              matcher.matchedConstantIndex(SIGNATURE_INDEX),
              new ConstantVisitor() {
                @Override
                public void visitAnyConstant(Clazz clazz, Constant constant) {}
          ...
          if (result.callableOwnerClass != null) {
            result.callableOwnerClass.kotlinMetadataAccept(
                new KotlinMetadataVisitor() {
                  @Override
                  public void visitAnyKotlinMetadata(Clazz clazz, KotlinMetadata kotlinMetadata) {}

                  @Override
                  public void visitKotlinDeclarationContainerMetadata(
                      Clazz clazz, KotlinDeclarationContainerMetadata declarationContainer) {
                    result.callableOwnerMetadata = declarationContainer;
                  }
                });

            if (result.callableName != null && result.callableSignature != null) {
  ...
  /**
   * Helper class to load the callable reference information from the existing implementations in a
   * class generated by Kotlin <= 1.3.
   */
  private static class CallableReferenceInfoLoader1dot3
      implements ClassVisitor, MemberVisitor, ConstantVisitor, KotlinMetadataVisitor {
    ...
    public void visitProgramMethod(ProgramClass programClass, ProgramMethod programMethod) {
      ...
      programMethod.accept(
          programClass,
          new AllAttributeVisitor(new AllInstructionVisitor(new InstructionConstantVisitor(this))));
    }
    ...
    public void visitStringConstant(Clazz clazz, StringConstant stringConstant) {
      if (currentMethod.equals(
          REFLECTION.GETSIGNATURE_METHOD_NAME + REFLECTION.GETSIGNATURE_METHOD_DESC)) {
        result.callableSignature = stringConstant.getString(clazz);
      } else if (currentMethod.equals(
          REFLECTION.GETNAME_METHOD_NAME + REFLECTION.GETNAME_METHOD_DESC)) {
        result.callableName = stringConstant.getString(clazz);
      }
    }
  ...
  private static class FunctionReferenceInfoInitializer implements KotlinFunctionVisitor {
    ...
    private FunctionReferenceInfoInitializer(
        Clazz ownerClass,
  ...
  public static class LocalVariableReferenceInfoInitializer implements KotlinMetadataVisitor {
    ...
    LocalVariableReferenceInfoInitializer(
        Clazz ownerClass,
        KotlinDeclarationContainerMetadata ownerMetadata,
        KotlinSyntheticClassKindMetadata syntheticClassKindMetadata,
        String name,
  ...
  public static class OptimizedCallableReferenceFilter implements ClassVisitor {
    ...
    public OptimizedCallableReferenceFilter(
        ClassVisitor optimizedVisitor, ClassVisitor nonOptimizedVisitor) {
      ...
      boolean isOptimized =
          clazz.findMethod(REFLECTION.GETNAME_METHOD_NAME, REFLECTION.GETNAME_METHOD_DESC) == null
              && clazz.findMethod(
                  ...
                  == null
              && clazz.findMethod(
                      REFLECTION.GETOWNER_METHOD_NAME, REFLECTION.GETSIGNATURE_METHOD_DESC)
                  == null;
