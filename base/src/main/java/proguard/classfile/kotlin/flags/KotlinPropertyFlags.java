...
public class KotlinPropertyFlags implements KotlinFlags {
   ...
   * because a property with a suitable signature exists in a supertype. This flag is not written by
   * the Kotlin compiler and its effects are unspecified.
   */
  public boolean isFakeOverride;

  /**
   * A member kind flag, signifying that the corresponding property exists in the containing class
   * because it has been produced by interface delegation (delegation "by"). Kotlinc never writes
   * this flag.
   */
  ...
  /**
   * Signifies that the corresponding property has a constant value. On JVM, this flag allows an
   * optimization similarly to [F.HAS_ANNOTATIONS]: constant values of properties are written to the
   * bytecode directly, and this flag can be used to avoid reading the value from the bytecode in
