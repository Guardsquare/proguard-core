...
public class MemberNameFilter implements MemberVisitor {
   ...
   *
   * @param regularExpression the regular expression against which member names will be matched.
  ...
  public MemberNameFilter(
      String regularExpression, WildcardManager wildcardManager, MemberVisitor memberVisitor) {
    this(new ListParser(new NameParser(wildcardManager)).parse(regularExpression), memberVisitor);
  }

  /**
   * /** Creates a new MemberNameFilter.
   *
   * @param regularExpressionMatcher the regular expression against which member names will be
   *     matched.
   * @param memberVisitor the <code>MemberVisitor</code> to which visits will be delegated.
   */
