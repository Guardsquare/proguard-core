...
/**
 * This {@link MemberVisitor} delegates its visits to one of three delegates, depending on whether
 * the visited method is:
 *
 * <p>- a constructor - a constructor that calls a super constructor - or another method.
 *
 * @author Johan Leys
 */
public class ConstructorMethodFilter
  ...
  public ConstructorMethodFilter(
      MemberVisitor constructorVisitor, MemberVisitor otherMethodVisitor) {
    this(constructorVisitor, constructorVisitor, otherMethodVisitor);
  }

  public ConstructorMethodFilter(
      MemberVisitor superCallingConstructorVisitor,
  ...
  public void visitAnyInstruction(
    ...
    if (invokeMatcher.isMatching()) {
      AnyMethodrefConstant methodrefConstant =
          (AnyMethodrefConstant)
              ((ProgramClass) clazz).getConstant(invokeMatcher.matchedArgument(FIELD_INDEX));
