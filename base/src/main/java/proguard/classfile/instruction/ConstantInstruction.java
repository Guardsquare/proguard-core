...
public class ConstantInstruction extends Instruction implements ConstantVisitor {
   ...
   *
   * <p>In particular, this takes into account that an ldc with a class, method type or method ref
   * constant operand may throw exceptions while other constants may not.
   */
  @Override
  ...
  public byte canonicalOpcode() {
    ...
    return opcode == Instruction.OP_LDC_W ? Instruction.OP_LDC : opcode;
  }
  ...
  protected void writeInfo(byte[] code, int offset) {
    ...
    if (requiredConstantIndexSize() > constantIndexSize) {
      throw new IllegalArgumentException(
          "Instruction has invalid constant index size (" + this.toString(offset) + ")");
    }
  ...
  private int constantIndexSize() {
    return opcode == Instruction.OP_LDC ? 1 : 2;
  }

  /** Returns the constant size for this instruction. */
  private int constantSize() {
    return opcode == Instruction.OP_MULTIANEWARRAY
        ? 1
        : opcode == Instruction.OP_INVOKEDYNAMIC || opcode == Instruction.OP_INVOKEINTERFACE
            ? 2
            : 0;
  }

  /** Computes the required constant pool index size for this instruction's constant pool index. */
  private int requiredConstantIndexSize() {
    return (constantIndex & 0xff) == constantIndex
        ? 1
        : (constantIndex & 0xffff) == constantIndex ? 2 : 4;
  }
