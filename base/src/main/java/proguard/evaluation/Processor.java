...
/**
 * This {@link InstructionVisitor} executes the instructions that it visits on a given local
...
public class Processor implements InstructionVisitor {
   ...
   * @param alwaysCast a flag that specifies whether downcasts or casts of null values should always
   *     be performed.
   */
  public Processor(
      Variables variables,
    ...
    switch (simpleInstruction.opcode) {
      ...
      case Instruction.OP_NEWARRAY:
        ...
        stack.push(
            valueFactory.createArrayReferenceValue(
                String.valueOf(
                    InstructionUtil.internalTypeFromArrayType((byte) simpleInstruction.constant)),
                null,
                arrayLength));
        break;
      ...
      default:
        throw new IllegalArgumentException(
            "Unknown simple instruction [" + simpleInstruction.opcode + "]");
    }
  }
  ...
  public void visitConstantInstruction(
    ...
    switch (constantInstruction.opcode) {
      ...
      case Instruction.OP_INVOKEDYNAMIC:
        invocationUnit.invokeMember(
            clazz, method, codeAttribute, offset, constantInstruction, stack, variables);
        break;
      ...
      case Instruction.OP_ANEWARRAY:
        {
          ReferenceValue arrayType =
              constantValueFactory.constantValue(clazz, constantIndex).referenceValue();

          stack.push(
              valueFactory.createArrayReferenceValue(
                  arrayType.internalType(), arrayType.getReferencedClass(), stack.ipop()));
          break;
        }
      ...
      case Instruction.OP_CHECKCAST:
        {
          ...
          stack.push(
              stack
                  .apop()
                  .cast(type.getType(), type.getReferencedClass(), valueFactory, alwaysCast));
          break;
        }
      ...
      case Instruction.OP_INSTANCEOF:
        {
          ...
          stack.push(
              instanceOf == Value.NEVER
                  ? valueFactory.createIntegerValue(0)
                  : instanceOf == Value.ALWAYS
      ...
      default:
        throw new IllegalArgumentException(
            "Unknown constant pool instruction [" + constantInstruction.opcode + "]");
    }
  }
    ...
    switch (variableInstruction.opcode) {
      ...
      case Instruction.OP_IINC:
        variables.store(
            ...
            variables
                .iload(variableIndex)
                .add(valueFactory.createIntegerValue(variableInstruction.constant)));
        break;
      ...
      case Instruction.OP_RET:
        ...
        branchUnit.branch(
            ...
            instructionOffsetValue.instructionOffset(
                instructionOffsetValue.instructionOffsetCount() - 1));
        break;

      default:
        throw new IllegalArgumentException(
            "Unknown variable instruction [" + variableInstruction.opcode + "]");
    }
  }
  ...
  public void visitBranchInstruction(
    ...
    branchUnit.branchConditionally(
        clazz, codeAttribute, offset, offset + branchInstruction.length(offset), Value.MAYBE);
    ...
    switch (branchInstruction.opcode) {
      ...
      case Instruction.OP_IFICMPEQ:
        branchUnit.branchConditionally(
            clazz, codeAttribute, offset, branchTarget, stack.ipop().equal(stack.ipop()));
        break;

      case Instruction.OP_IFICMPNE:
        branchUnit.branchConditionally(
            clazz, codeAttribute, offset, branchTarget, stack.ipop().notEqual(stack.ipop()));
        break;

      case Instruction.OP_IFICMPLT:
        // Note that the stack entries are popped in reverse order.
        branchUnit.branchConditionally(
            clazz, codeAttribute, offset, branchTarget, stack.ipop().greaterThan(stack.ipop()));
        break;

      case Instruction.OP_IFICMPGE:
        // Note that the stack entries are popped in reverse order.
        branchUnit.branchConditionally(
            clazz, codeAttribute, offset, branchTarget, stack.ipop().lessThanOrEqual(stack.ipop()));
        break;

      case Instruction.OP_IFICMPGT:
        // Note that the stack entries are popped in reverse order.
        branchUnit.branchConditionally(
            clazz, codeAttribute, offset, branchTarget, stack.ipop().lessThan(stack.ipop()));
        break;
      ...
      case Instruction.OP_IFACMPEQ:
        branchUnit.branchConditionally(
            clazz, codeAttribute, offset, branchTarget, stack.apop().equal(stack.apop()));
        break;

      case Instruction.OP_IFACMPNE:
        branchUnit.branchConditionally(
            clazz, codeAttribute, offset, branchTarget, stack.apop().notEqual(stack.apop()));
        break;
      ...
      case Instruction.OP_JSR_W:
        stack.push(new InstructionOffsetValue(offset + branchInstruction.length(offset)));
        branchUnit.branch(clazz, codeAttribute, offset, branchTarget);
        break;

      case Instruction.OP_IFNULL:
        branchUnit.branchConditionally(
            clazz, codeAttribute, offset, branchTarget, stack.apop().isNull());
        break;

      case Instruction.OP_IFNONNULL:
        branchUnit.branchConditionally(
            clazz, codeAttribute, offset, branchTarget, stack.apop().isNotNull());
        break;

      default:
        throw new IllegalArgumentException(
            "Unknown branch instruction [" + branchInstruction.opcode + "]");
    }
  }
  ...
  public void visitTableSwitchInstruction(
    ...
    branchUnit.branchConditionally(
        clazz, codeAttribute, offset, offset + tableSwitchInstruction.defaultOffset, Value.MAYBE);

    for (int index = 0; index < tableSwitchInstruction.jumpOffsets.length; index++) {
      int conditional =
          indexValue.equal(valueFactory.createIntegerValue(tableSwitchInstruction.lowCase + index));
  ...
  public void visitLookUpSwitchInstruction(
    ...
    // Maybe branch to the default offset.
    branchUnit.branchConditionally(
        clazz, codeAttribute, offset, offset + lookUpSwitchInstruction.defaultOffset, Value.MAYBE);

    for (int index = 0; index < lookUpSwitchInstruction.jumpOffsets.length; index++) {
      int conditional =
          indexValue.equal(valueFactory.createIntegerValue(lookUpSwitchInstruction.cases[index]));
  ...
  private void handleArrayException(Value toReplace, ReferenceValue currentValue) {
    Value unknownArray =
        valueFactory.createArrayReferenceValue(
            currentValue.getType() == null ? null : currentValue.getType().substring(1),
