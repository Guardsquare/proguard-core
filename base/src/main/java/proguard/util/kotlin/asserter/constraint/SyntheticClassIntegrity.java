...
public class SyntheticClassIntegrity extends AbstractKotlinMetadataConstraint
  ...
  public void visitKotlinSyntheticClassMetadata(
    ...
    AssertUtil util =
        new AssertUtil("Synthetic class", reporter, programClassPool, libraryClassPool);

    switch (kotlinSyntheticClassKindMetadata.flavor) {
      case DEFAULT_IMPLS:
        if (!clazz.getName().endsWith(DEFAULT_IMPLEMENTATIONS_SUFFIX)) {
          reporter.report(
              "Default implementations class name does not end with "
    ...
    if (clazz.extendsOrImplements(REFLECTION.CALLABLE_REFERENCE_CLASS_NAME)) {
      ...
      util.reportIfNullReference(
          "callable reference info", kotlinSyntheticClassKindMetadata.callableReferenceInfo);

      kotlinSyntheticClassKindMetadata.callableReferenceInfoAccept(
          new CallableReferenceInfoVisitor() {
            @Override
            public void visitAnyCallableReferenceInfo(CallableReferenceInfo callableReferenceInfo) {
              util.setParentElement(
                  "Synthetic callable reference ("
                      + callableReferenceInfo.getClass().getSimpleName()
                      + ")");
            ...
            @Override
            public void visitLocalVariableReferenceInfo(
