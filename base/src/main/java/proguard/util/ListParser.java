...
/**
 * This {@link StringParser} can create {@link StringMatcher} instances for regular expressions. The
 ...
 * match is returned, without considering any subsequent entries in the list. The creation of {@link
 * StringMatcher} instances for the entries is delegated to the given {@link StringParser}.
 *
 * <p>For example, "*.xml,*.jpg" matches all strings with the given extensions; "!dummy" matches all
 * strings except "dummy"; "!dummy.xml,*.xml" matches all strings with .xml, except "dummy.xml";
...
public class ListParser implements StringParser {
  ...
  public StringMatcher parse(List<String> regularExpressions) {
    ...
    for (int index = regularExpressions.size() - 1; index >= 0; ) {
      ...
      // later on.
      int lastNegatedIndex = lastNegatedIndex(regularExpressions, index, isNegated);

      StringMatcher[] stringMatchers = parseEntries(regularExpressions, lastNegatedIndex, index);
      ...
      listMatcher =
          listMatcher == null
              ? stringMatcher
              : isNegated
                  ? new AndMatcher(stringMatcher, listMatcher)
  ...
  /** Creates a StringMatcher for the given regular expression, stripping any negator. */
