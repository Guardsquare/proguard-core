...
/**
 * This {@link DataEntryWriter} delegates to a given {@link DataEntryWriter}, or failing that, to
 * another given {@link DataEntryWriter}.
 *
...
public class CascadingDataEntryWriter implements DataEntryWriter {
   ...
   *
   * @param dataEntryWriter1 the DataEntryWriter to which the writing will be delegated first.
   * @param dataEntryWriter2 the DataEntryWriter to which the writing will be delegated, if the
   *     first one can't provide an output stream.
   */
  public CascadingDataEntryWriter(
      DataEntryWriter dataEntryWriter1, DataEntryWriter dataEntryWriter2) {
  ...
  public OutputStream createOutputStream(DataEntry dataEntry) throws IOException {
    ...
    OutputStream outputStream = dataEntryWriter1.createOutputStream(dataEntry);

    // Return it, if it's not null. Otherwise try to get an output stream
    // from the second data entry writer.
    return outputStream != null ? outputStream : dataEntryWriter2.createOutputStream(dataEntry);
  }
