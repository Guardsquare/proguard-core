...
/**
 * This class can create DataEntryWriter instances based on class paths. The writers will wrap the
 * output in the proper apks, jars, aars, wars, ears, and zips. \
 *
...
public class DataEntryWriterFactory {
  ...
  private static final boolean ENABLE_ZIP64_SUPPORT =
      System.getProperty("enable.zip64.support") != null;
  ...
  private /*static*/ final String[][] JMOD_PREFIXES =
      new String[][] {{CLASS_FILE_PATTERN, CLASS_FILE_PREFIX}};

  private /*static*/ final String[][] WAR_PREFIXES =
      new String[][] {{CLASS_FILE_PATTERN, CLASS_FILE_PREFIX}};
   ...
   * @param pageAlignNativeLibs specifies whether to align native libraries at page boundaries.
   * @param mergeAarJars specifies whether to merge all jars in an Android library aar into a single
   *     jar.
   ...
   * @param uncompressedAlignment the desired alignment for the data of uncompressed entries.
   * @param pageAlignNativeLibs specifies whether to align native libraries at page boundaries.
   * @param mergeAarJars specifies whether to merge all jars in an Android app bundle into a single
   *     jar.
   * @param privateKeyEntries optional private keys to sign jars.
   * @param alternativeClassDataEntryWriterProvider optional, to provide an alternative class
   *     writer, instead of the default {@link ClassDataEntryWriter}.
   */
  public DataEntryWriterFactory(
      ClassPool programClassPool,
      ResourceFilePool resourceFilePool,
  ...
  public DataEntryWriter createDataEntryWriter(
      ClassPath classPath, int fromIndex, int toIndex, DataEntryWriter extraDataEntryWriter) {
    ...
    for (int index = toIndex - 1; index >= fromIndex; index--) {
      ...
      boolean addCheckingJarWriter = !outputFileOccurs(entry, classPath, 0, index);
      ...
      writer =
          createClassPathEntryWriter(
              entry, writer, extraDataEntryWriter, addCheckingJarWriter, closeCachedJarWriter);
    }

    return writer;
  }

  private boolean outputFileOccurs(
      ClassPathEntry entry, ClassPath classPath, int startIndex, int endIndex) {
    File file = entry.getFile();

    for (int index = startIndex; index < endIndex; index++) {
      ClassPathEntry classPathEntry = classPath.get(index);
      if (classPathEntry.isOutput() && classPathEntry.getFile().equals(file)) {
        return true;
      }
    }
  ...
  /**
   * Creates a DataEntryWriter that can write to the given class path entry, or delegate to another
   * DataEntryWriter if its filters don't match.
   */
  private DataEntryWriter createClassPathEntryWriter(
    ...
    DataEntryWriter writer = isFile ? new FixedFileWriter(file) : new DirectoryWriter(file);

    if (isDex) {
      // A dex file can't contain resource files.
      writer =
          new FilteredDataEntryWriter(
              new DataEntryNameFilter(new ExtensionMatcher(".dex")), writer);
    } else {
      ...
      DataEntryWriter classWriter =
          new ClassDataEntryWriter(programClassPool, new NonClosingDataEntryWriter(writer));
  ...
  /** Wraps the given DataEntryWriter in a ZipWriter, filtering and signing if necessary. */
  private DataEntryWriter wrapInJarWriter(
      File file,
    ...
    StringMatcher pageAlignmentFilter =
        pageAlignNativeLibs ? new FileNameParser().parse("lib/*/*.so") : null;
    ...
    // Do we have a cached writer?
      ...
      // Sign the jar.
      zipWriter =
          wrapInSignedJarWriter(
              writer, extraDataEntryWriter, isAab, jarHeader, pageAlignmentFilter);
        ...
        for (int index = prefixes.length - 1; index >= 0; index--) {
          ...
          zipWriter =
              new FilteredDataEntryWriter(
                  ...
                  new PrefixAddingDataEntryWriter(prefix, prefixlessJarWriter),
                  zipWriter);
        }
      }
    ...
    new FilteredDataEntryWriter(
        new DataEntryParentFilter(
            new DataEntryNameFilter(new ExtensionMatcher(jarFilterExtension))),
        ...
        isJar ? zipWriter : writer);
  }

  /** Wraps the given DataEntryWriter in a ZipWriter, signing if necessary. */
  private DataEntryWriter wrapInSignedJarWriter(
      DataEntryWriter writer,
      DataEntryWriter extraDataEntryWriter,
      boolean isAab,
      byte[] jarHeader,
