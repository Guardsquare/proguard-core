...
public class FilteredDataEntryWriter implements DataEntryWriter {
   ...
   * @param acceptedDataEntryWriter the DataEntryWriter to which the writing will be delegated if
   *     the filter accepts the data entry. May be <code>null</code>.
   */
  public FilteredDataEntryWriter(
      DataEntryFilter dataEntryFilter, DataEntryWriter acceptedDataEntryWriter) {
   ...
   * @param acceptedDataEntryWriter the DataEntryWriter to which the writing will be delegated if
   *     the filter accepts the data entry. May be <code>null</code>.
   * @param rejectedDataEntryWriter the DataEntryWriter to which the writing will be delegated if
   *     the filter does not accept the data entry. May be <code>null</code>.
   */
  public FilteredDataEntryWriter(
  ...
  public boolean sameOutputStream(DataEntry dataEntry1, DataEntry dataEntry2) throws IOException {
    ...
    return accepts1
        ? !accepts2
            ...
            || acceptedDataEntryWriter.sameOutputStream(dataEntry1, dataEntry2)
        : accepts2
            || rejectedDataEntryWriter == null
            || rejectedDataEntryWriter.sameOutputStream(dataEntry1, dataEntry2);
  }
